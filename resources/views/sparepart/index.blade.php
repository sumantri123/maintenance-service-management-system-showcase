	...
	<div class="card-body">
		<div class="border border-primary p-3 rounded">
			<div id="invoice">            
				<div class="invoice">	
					<div class="row">
						<div class="col company-details">																												@if(hasPermission('sparepart','tambah',Session::get('role')))
					...
					<div id="error-validation"></div>
						<div class="row g-3">                        							
							<div class="col-md-12 col-sm-12">
								...
								<select id="tipe" name="tipe" class="{{$data['classForm']}} clear" required>
							...
							<div class="col-md-12 col-sm-12">
								<label for="inputNote" class="form-label"><b>Nama Sparepart</b></label>                            
								<input type="text" class="{{$data['classForm']}}" id="nama" name="nama" required/>
							</div>							                         
						</div>                    
                </div>
...
@push('scripts')    
<script>		
	var routeData = "{{ route('sparepart.data') }}";		
	var routeNonAktif = "{{ route('sparepart.nonaktif') }}";		
	var routeAktif = "{{ route('sparepart.aktif') }}";		
	var routeHapus = "{{ route('sparepart.hapus', ['id' => 'PARAMETER']) }}"; 		
	var routeSimpan = "{{ route('sparepart.save') }}";		
	var routeUbah = "{{ route('sparepart.ubah') }}";			
</script>
<script src="{{ asset('additional/js/setup/sparepart.js') }}"></script>
@endpush